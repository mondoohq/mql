...
func TestDetect_AssetName(t *testing.T) {
	...
	t.Run("keeps a caller-supplied name", func(t *testing.T) {
		...
		assert.Contains(t, asset.PlatformIds[0], "//platformid.api.mondoo.app/runtime/kustomize/hash/")
		assert.Equal(t, "kustomize", asset.Platform.Name)
	})
}
