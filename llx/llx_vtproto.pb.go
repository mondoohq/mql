...
func (m *CoverageGap) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *CoverageGap) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *CoverageGap) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
...
func (m *Result) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.CoverageGaps) > 0 {
		for iNdEx := len(m.CoverageGaps) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.CoverageGaps[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
	...
	if m.ErrorDetail != nil {
		size, err := m.ErrorDetail.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *CoverageGap) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *Result) SizeVT() (n int) {
	...
	if m.ErrorDetail != nil {
		l = m.ErrorDetail.SizeVT()
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
	}
	if len(m.CoverageGaps) > 0 {
		for _, e := range m.CoverageGaps {
			l = e.SizeVT()
			n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
	...
	for iNdEx < l {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Scope |= ErrorScope(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 5:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RetryAfterMs |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *CoverageGap) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CoverageGap: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CoverageGap: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
func (m *Result) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			...
			if err := m.ErrorDetail.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field CoverageGaps", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.CoverageGaps = append(m.CoverageGaps, &CoverageGap{})
			if err := m.CoverageGaps[len(m.CoverageGaps)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
