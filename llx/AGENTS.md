...
- **A partial result is not an error** (ADR 046 §8). The parts that could not be read ride in `RawData.CoverageGaps`, never in `Error`, which every consumer reads as "failed". Builtins don't handle gaps: the executor collects them along the code graph when it delivers a result. Never write gaps onto a `RawData` in place, since values are shared through the caches and can be `NilData`; use `WithCoverageGaps`, which copies.
