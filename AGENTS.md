...
- Exception until ADR 046 settles lists that carry both data and an error: inside a per-region (or per-project, per-subscription) loop, a denied partition is still logged and skipped, so the other partitions' data survives.
