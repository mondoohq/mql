...
- Exception: a refusal inside a per-region (or per-project, per-subscription) loop is a partial result (ADR 046 §8), not a field error. Keep the other partitions' data. Until `llx.Partial` lands, log and skip the denied partition as today; never fail the whole list for it.
