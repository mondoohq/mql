...
- Name the permission at the call site when you know it. When you don't, the cloud's classifier falls back to the provider's `*.permissions.json` through `plugin.PermissionIndex`: `LookupCaller(1, operation)` gives the calling file's permissions, narrowed to the operation the error names.
- `llx.WithScope(llx.ErrorScope_ERROR_SCOPE_ASSET, "")` only when every later call on the asset fails the same way (a 401, an expired credential). The runtime then stops calling the provider for that connection and answers every remaining field with this error, so a wrong `ASSET` scope wipes out the whole asset's data.
- A genuine absence stays a null. `NotFound` is for "could not answer", never for "the answer is nothing".
- Exception: a refusal inside a per-region (or per-project, per-subscription) loop is a partial result (ADR 046 §8), not a field error. Keep the other partitions' data. `llx.Partial` exists, but loops convert only in ADR 046 step 11, once mql and cnspec show the gaps; until then log and skip the denied partition as today, and never fail the whole list for it.
