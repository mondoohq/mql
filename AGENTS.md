...
Rules for working in mql. Procedures live in the skills (§1.5), reference material in [DEVELOPMENT.md](DEVELOPMENT.md), subtree rules in scoped files like `llx/AGENTS.md`.
...
Patterns and samples: `.agents/skills/new-resource/references/implementation-patterns.md`. Rules:
...
- A singular resource accessor returning `nil, nil` must first set `a.Field.State = plugin.StateIsSet | plugin.StateIsNull`, or the runtime doesn't know the field resolved and may panic or re-fetch. Every path counts: empty ID, nil response, unset optional. A refused call is not one of them, it returns an error (next bullets). Slices, maps, scalars unaffected.
...
- A refusal is an error, never a null (ADR 046 §3). The provider's classifiers (`Is400AccessDeniedError`, `isServiceDisabled`, `isAzureAccessDenied`, ...) decide the kind; a true answer wraps the SDK error instead of returning `nil, nil`: `return nil, llx.Forbidden(err, llx.WithPermissions("ec2:DescribeInstances"))`. In aws, `return nil, classifyAwsError(err, "ec2:DescribeInstances")` runs every aws check and then the HTTP status mapping (`plugin.ClassifyHTTPStatus`). The kinds: `Unauthenticated` (401), `Forbidden` (403, includes a missing `sudo`), `NotFound` (could not answer because what it had to read is missing), `NotApplicable` (API not enabled, not in this plan or region), `Gone` (retired API), `TooManyRequests` (with `llx.WithRetryAfter`), `Unavailable` (503, connection refused; other 5xx and timeouts stay unclassified), `MalformedData` (the target's answer doesn't parse). Classify by what the target said, not the status alone (GCP's 403 "API not enabled" is `NotApplicable`). Pick the narrowest kind that is true; unsure means unclassified, return the error as is. A wrong kind is worse than none.
- A genuine absence stays a null. `NotFound` is for "could not answer", never for "the answer is nothing".
- Exception: a refusal inside a per-region (or per-project, per-subscription) loop is a partial result (ADR 046 §8), not a field error. Keep the other partitions' data. `llx.Partial` exists, but loops convert only in ADR 046 step 10, once mql and cnspec show the gaps; until then log and skip the denied partition as today, and never fail the whole list for it.
...
**Error kinds (ADR 046).** A classified failure is an `*llx.Error`; read it with `llx.KindOf(err)` or `errors.Is(err, llx.ErrForbidden)`, never by matching the message. It crosses the process boundary as `ErrorDetail` on `DataRes` and `llx.Result`. Rebuilding an error from a wire message goes through `llx.ErrorFromDetail`; a bare `errors.New(res.Error)` drops the kind.
